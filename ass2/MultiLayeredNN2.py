...
class NN:
	...
	def training(self, epochs, batch_size, lr):
		...
		for epoch in range(epochs):
			...
			# Shuffles the trainings set for each epoch
			...
			for i in range(num_batches_per_epoch):
				...
				self.weights = NN.gradient_descent(x_batch, outputs, targets, self.weights, learning_rate)
	...
	def gradient_descent(x, outputs, targets, weights, learning_rate):
		...
		# NN.check_gradient(outputs, targets, weights, 0, gradient_w[0])
	...
	@staticmethod
	def check_gradient(outputs, targets, weights, layer, computed_gradient, epsilon=1e-2):
		...
		for k in range(weights[layer].shape[0]):
			for j in range(weights[layer].shape[1]):
				...
				loss1 = NN.cross_entropy(outputs, targets)
				new_weight2[layer][k, j] -= epsilon
				loss2 = NN.cross_entropy(outputs, targets)
				dw[k, j] = (loss1 - loss2) / (2 * epsilon)

		maximum_absolute_difference = abs(computed_gradient - dw).max()
		assert maximum_absolute_difference <= epsilon ** 2, "Absolute error was: {}".format(
			maximum_absolute_difference)

	@staticmethod
	def forward_pass(x, weights):
		"""Forward pass through the network. Converting inputs to outputs."""
		a = x
		for layer in range(len(weights)):
			z = a.dot(weights[layer])
			if layer == len(weights) - 1:
	...
	@staticmethod
	def softmax_derivative(z, outputs, targets):
		"""Derivative of Softmax activation function for output layer(s). To be used in backpropagation(???)"""
		return - np.multiply(z, (targets - outputs))
	...
	def momentum(z):
		"""Task 3d"""
		# TODO: Implement momentum as described in the paper.
		pass
